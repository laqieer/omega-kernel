...
void print_string(char* str, int x, int y, int color) {
    int x0 = x;
    while(*str != '\0')
    {
        if(*str == '\n')
...
*/
print_string:	@; r0 打印的字符串地址， r1, X  ， r2, Y , r3 颜色
...
/* //ARM gcc 8.2 -Os
void printchar(char c, int x, int y, int color);
void print_string_bank(char* str_bank, int x, int y, int color) {
    int x0 = x;
    while(*str_bank != '\0' || *(str_bank + 1) != '\0')
    {
        if(*str_bank == '\n' || *str_bank == '\0')
        {
            y += 16;
